#!/usr/bin/env python3
"""
Run era-balanced weighted k-medoids clustering on the final Facebook annual RFE class profiles.

Final Facebook setup:
- years kept: 2019, 2020, 2021, 2022, 2026
- top-level comments only
- users with at least 2 comments in the year
- RFE features:
  - recency_class from scaled posts-since-last-comment thresholds
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Cluster the final Facebook annual RFE classification with era-balanced weighted k-medoids."
    )
    parser.add_argument(
        "--input",
        default=SCRIPT_DIR / "fb_comments_RFE_macro_top_level_annual_2017_2022_2026_min2_absolute_rf_classified_candidate.csv",
        type=Path,
        help="Input classified Facebook annual RFE CSV.",
    )
    parser.add_argument(
        "--ks",
        default="3,4,5,6",
    ...
    parser.add_argument(
        "--output-models",
        default=SCRIPT_DIR / "fb_comments_rfe_kmedoids_model_selection_candidate.csv",
        type=Path,
