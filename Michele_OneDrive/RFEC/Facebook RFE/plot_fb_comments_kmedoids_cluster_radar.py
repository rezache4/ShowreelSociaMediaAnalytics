#!/usr/bin/env python3
"""
Plot per-cluster radar charts that compare Facebook annual RFE centroid evolution across selected years.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot Facebook annual RFE radar charts of cluster centroid evolution by year."
    )
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        "--output-dir",
        default=SCRIPT_DIR / "fb_kmedoids_radar_plots_candidate",
        type=Path,
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=YEAR_ORDER,
        help="Years to include in the radar chart. Default: 2019 2020 2021 2022 2026",
    )
    parser.add_argument(
...
def year_color_map(years: list[str]) -> dict[str, str]:
    fallback_palette = ["#1d3557", "#e76f51", "#2a9d8f", "#8d99ae", "#6d597a", "#f4a261"]
    colors: dict[str, str] = {}
    for index, year in enumerate(years):
        colors[year] = YEAR_COLORS.get(year, fallback_palette[index % len(fallback_palette)])
    return colors
...
def plot_radar_panel(
    ...
    for axis, cluster_id in zip(axes_array, cluster_ids):
        ...
        for year in years:
            ...
            axis.plot(closed_angles, closed_values, color=colors[year], linewidth=2, label=year)
            axis.fill(closed_angles, closed_values, color=colors[year], alpha=0.22)

    for axis in axes_array[n_clusters:]:
        axis.set_visible(False)

    legend_handles = [
        plt.Line2D([0], [0], color=colors[year], linewidth=2, label=year) for year in years
    ]
    fig.legend(
    ...
    fig.suptitle("Facebook annual RFE Cluster Centroids by Year", fontsize=17, y=1.02)
...
def save_tables(
    ...
    raw_path = output_dir / f"fb_comments_kmedoids_k{k}_cluster_year_centroids_raw_{suffix}.csv"
    ...
    scaled_path = output_dir / f"fb_comments_kmedoids_k{k}_cluster_year_centroids_scaled_{suffix}.csv"
...
def main() -> None:
    ...
    suffix = "_".join(years)
    radar_path = args.output_dir / f"fb_comments_kmedoids_k{args.k}_cluster_radar_by_year_{suffix}.png"
    raw_path, scaled_path = save_tables(
        centroids_raw,
        centroids_scaled,
        metadata,
        years,
    ...
    plot_radar_panel(centroids_scaled, ordered_ids, metadata, years, colors, radar_path)
