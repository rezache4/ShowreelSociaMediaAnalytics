#!/usr/bin/env python3
"""
Plot Facebook RFE metric distributions and cumulative distributions by era.

Outputs:
- a panel with per-era distributions for the selected metrics
- a panel with per-era empirical cumulative distributions for the same metrics

Threshold lines are optional and use the final global thresholds.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot Facebook RFE distributions and cumulative distributions by era."
    )
    parser.add_argument(
        "--macro",
    ...
    parser.add_argument(
        "--output-dir",
        default=SCRIPT_DIR / "fb_rfe_plots_top_level_annual_2017_2022_2026_min2_absolute_rf_candidate",
        type=Path,
...
def metric_xlabel(df: pd.DataFrame, metric: str) -> str:
    if metric == "recency":
        max_value = pd.to_numeric(df[metric], errors="coerce").max()
        if pd.notna(max_value) and float(max_value) > 1.5:
            return "Posts since last comment"
    if metric == "frequency":
        max_value = pd.to_numeric(df[metric], errors="coerce").max()
        if pd.notna(max_value) and float(max_value) > 1.5:
            frequency_values = pd.to_numeric(df["frequency"], errors="coerce")
            for column, label in (
                ("distinct_posts_count", "Distinct posts commented in year"),
                ("comment_count", "Comments in year"),
            ):
                if column not in df.columns:
                    continue
                base_values = pd.to_numeric(df[column], errors="coerce")
                mask = frequency_values.notna() & base_values.notna()
                if mask.any() and np.allclose(
                    frequency_values.loc[mask].to_numpy(dtype=float),
                    base_values.loc[mask].to_numpy(dtype=float),
                ):
                    return label
    return METRIC_XLABELS[metric]


def load_macro(path: Path, metrics: list[str]) -> tuple[pd.DataFrame, list[str]]:
    usecols = set(metrics) | {"era", "era_start", "comment_count", "distinct_posts_count"}
...
def percentile_annotation_text(
    ...
    lines = [f"T1 -> overall P{summary['overall']['threshold_1_share'] * 100:.1f}"]
    t1_items = [f"{era} P{summary[era]['threshold_1_share'] * 100:.1f}" for era in era_order]
    ...
    t2_items = [f"{era} P{summary[era]['threshold_2_share'] * 100:.1f}" for era in era_order]
...
def threshold_upper_bound(threshold_spec: dict[str, object]) -> float:
    return max(threshold_spec["values"])
...
def add_threshold_lines(
    ...
    for threshold, style in zip(threshold_spec["values"], THRESHOLD_STYLES):
        ax.axvline(
            threshold,
            color=style["color"],
...
def figure_legend_handles(
    ...
    handles = [Line2D([0], [0], color=era_colors[era], linewidth=2.2, label=era) for era in era_order]
...
def plot_distribution_panel(
    ...
    era_order: list[str],
    era_colors: dict[str, object],
    thresholds: dict[str, dict[str, object]] | None,
    ...
    for ax, metric in zip(axes, metrics):
        ...
        for era in era_order:
            values = df.loc[df["era"] == era, metric].dropna().to_numpy(dtype=float)
        ...
        if thresholds is not None and percentile_summaries is not None:
        ...
        ax.set_xlabel(metric_xlabel(df, metric))
    ...
    fig.suptitle("Facebook RFE Distributions by Era", fontsize=16, y=0.98)
...
def plot_cumulative_panel(
    ...
    fig.suptitle("Facebook Cumulative RFE Distributions by Era", fontsize=16, y=0.98)
    fig.subplots_adjust(top=top, hspace=0.30, wspace=0.22)
    fig.savefig(output_path, dpi=220, bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    macro, era_order = load_macro(args.macro, METRICS)
    era_colors = build_era_colors(era_order)
    thresholds = load_thresholds(args.thresholds, era_order, METRICS) if args.thresholds else None
    percentile_summaries = (
        {metric: threshold_summary(macro, metric, thresholds[metric], era_order) for metric in METRICS}
        if thresholds is not None
        else None
    )

    distribution_path = args.output_dir / "fb_rfe_distributions_by_era.png"
    cumulative_path = args.output_dir / "fb_rfe_cumulative_distributions_by_era.png"
